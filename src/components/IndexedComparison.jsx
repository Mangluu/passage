...
import { INDICES, INDEX_KEYS, INDEX_SHORT, indexView } from '../lib/world.js'

const BAR = { success: 'bg-success', warn: 'bg-warn', danger: 'bg-danger', ink3: 'bg-ink3' }
const TXT = { success: 'text-success', warn: 'text-warn', danger: 'text-danger', ink3: 'text-ink3', ink2: 'text-ink2' }
const CHIP = {
...
// The Tier B comparison — shown whenever at least one country isn't curated.
// It compares only the published indices we actually have for BOTH countries,
...
export default function IndexedComparison({ oInfo, dInfo }) {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-5 py-6">
      {/* Why this isn't the full briefing */}
      ...
      <div className="flex flex-col gap-4">
        {INDEX_KEYS.map((key) => (
          <IndexCompareCard key={key} k={key} oInfo={oInfo} dInfo={dInfo} />
        ))}
      </div>
      ...
      <div className="mt-8 border-t border-line pt-5 text-[12px] leading-relaxed text-ink3">
        ...
        briefing carries, and identity filters don’t change them. A value’s year is the year the source reflects.{' '}
...
function IndexCompareCard({ k, oInfo, dInfo }) {
  ...
  const delta = home && dst ? Math.round((dst.value - home.value) * 10) / 10 : null
  const chip =
    delta == null
      ? null
      : Math.abs(delta) < 5
        ...
        : delta > 0
          ? { text: `${delta} higher in ${dInfo.name}`, tone: 'success' }
          : { text: `${-delta} lower in ${dInfo.name}`, tone: 'danger' }

  return (
    <div className="card p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h3 className="font-serif text-[17px] font-semibold text-ink">{INDEX_SHORT[k]}</h3>
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between gap-2">
        ...
        {v ? (
          <span className="shrink-0 font-mono text-[12px] tnum">
            <span className={`font-semibold ${TXT[muted ? 'ink2' : v.tone]}`}>{v.value}</span>
      ...
      <div className="mt-1 h-2 overflow-hidden rounded-full bg-surface2">
        {v && <div className={`h-full rounded-full ${BAR[tone]}`} style={{ width: `${v.value}%` }} />}
      </div>
    </div>
  )
}
