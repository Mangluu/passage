...
import { signalsFor, ADVISORY_LABEL, ADVISORY_TONE } from '../lib/signals.js'
...
// "What changed recently" — the freshness layer a chatbot's frozen weights can't have.
export default function SignalsPanel({ dest, audiences }) {
  const { advisory, news, isFor, generatedAt } = signalsFor(dest.code, audiences)
  const advTone = advisory && advisory.level ? ADVISORY_TONE[advisory.level] : 'ink3'
  ...
  return (
    <section id="sec-signals" className="card scroll-mt-4 p-5">
      ...
      <p className="mb-4 text-[11.5px] leading-snug text-ink3">
        Recent, dated signals for {dest.name} — refreshed from public feeds{generatedAt ? ` (${generatedAt})` : ''}.
        These are things to <span className="text-ink2">check</span>, not verified facts.
      </p>
        ...
        >
          ...
          <span className="min-w-0">
            <span className={`text-[13px] font-semibold ${TONE_TXT[advTone]}`}>
              {advisory.level ? `Level ${advisory.level}` : 'Advisory'}
              {advisory.level && ADVISORY_LABEL[advisory.level] ? ` · ${ADVISORY_LABEL[advisory.level]}` : ''}
            </span>
            <span className="mt-0.5 block truncate font-mono text-[10px] text-ink3">US State Dept ↗</span>
          </span>
        </a>
      )}
      ...
      {news.length ? (
          ...
          {news.map((n, i) => (
            <li key={i} className="py-2.5 first:pt-0">
              <a href={n.url} target="_blank" rel="noreferrer" className="group block">
                ...
                <div className="mt-1 flex items-center gap-2 pl-0 font-mono text-[10px] text-ink3">
                  <span>{n.date || ''}</span>
                  <span>·</span>
        ...
        <p className="text-[13px] text-ink2">
          No recent identity-specific signals surfaced for {dest.name} this cycle. Absence of news is not evidence of
