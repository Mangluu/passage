...
export function ask({ question, origin, dest, audiences = [] }) {
  ...
  // ALL of the reader's relevant topics — not just the one keyword it happened to hit.
  let keys = direct
  // Broad safety/identity phrasing ("is it safe to be visibly me?") pulls the
  // reader's relevant topics. A question with NO topic match and NO safety intent
  // is genuinely out of scope — we refuse rather than pad the answer.
